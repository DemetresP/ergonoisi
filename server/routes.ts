  ...
  app.post("/api/tasks/ai-generate", isAuthenticated, async (req: any, res) => {
    try {
      ...
      for (const parsedTask of parsedTasks) {
        ...
        if (isAdmin(userEmail)) {
          if (parsedTask.assignee) (taskData as any).assignee = parsedTask.assignee;
          if (parsedTask.shootDate) (taskData as any).shootDate = parsedTask.shootDate;
          if (parsedTask.deliveryDate) (taskData as any).deliveryDate = parsedTask.deliveryDate;
        }
